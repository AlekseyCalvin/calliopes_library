...
const prosodyPluginUrl = WPURLS.prosody_plugin_url;
const imagesDir = `${prosodyPluginUrl}/images/`;
const correctAnswerUrl = imagesDir + "correct.png";
const incorrectAnswerUrl = imagesDir + "incorrect.png";
const expectedAnswerUrl = imagesDir + "expected.png";
const wipeStressUrl = imagesDir + "broom_small.png";
const stressDefaultUrl = imagesDir + "stress-default.png";
const feetDefaultUrl = imagesDir + "feet-default.png";
...
const getLineFeatureState = function (lineNumber, feature) {
  if (lineStates && lineStates[lineNumber - 1]) {
...
function checkmeter(lineNumber, lineGroupIndex) {
  var fullAnswer = $("#prosody-real-" + lineNumber + " span[answer]").attr(
    "answer"
  );
  if (!fullAnswer) {
...
function switchstress(shadowSyllable) {
  ...
  // Live checking on change
  // Note: We need to use a timeout here to match the timeouts above.
  const line = shadowSyllable.id[15];
  setTimeout(() => checkstress(parseInt(line)), 150);
}

function wipeStress(shadowLineElement) {
  var syllableNodes = shadowLineElement.getElementsByClassName(
    "prosody-shadowsyllable"
  ); //some of the child nodes are caesuras or texts
  ...
  for (var i = 0; i < syllableNodesLength; i++) {
    ...
    $("#checkstress" + shadowLineNumber + " img").attr("src", stressDefaultUrl);
  }
}
...
function checkstress(lineNumber) {
  ...
  var scheme = "";
  $("#prosody-real-" + lineNumber + " span[real]").each(function () {
  ...
  if (answerLength !== schemeLength) {
    $("#checkstress" + lineNumber + " img").attr("src", stressDefaultUrl);
    setLineFeatureState(lineNumber, feature, false);
    return;
  }

  var answer = $("#prosody-real-" + lineNumber)
    .data("real")
    .split("|");
  var correctAnswer = answer[0].replace(/-/g, "\u222a");
...
function verifyShowNote(lineNumber) {
  ...
  var verified = criteria.filter(
    (feature) => lineStates[lineNumber - 1][feature]
  ); // filter out the features that are not set to "true"
  return criteria.length === verified.length;
}

function showNote(lineNumber) {
  if (verifyShowNote(lineNumber)) {
    $("#displaynotebutton" + lineNumber)
      .show()
      .click(function () {
...
function switchfoot(syllableId) {
  ...
  $("#checkfeet" + shadowLineNumberSection + " img").attr(
    "src",
    feetDefaultUrl
  );
  ...
  // Live checking on change
  const line = syllableId[13];
  checkfeet(parseInt(line));
...
$(document).ready(function () {
  $(".prosody-line").append(
    "<div class='wipe-stress'><img src=" + wipeStressUrl + "></div>"
  );
  ...
  setTimeout(function () {
    ...
    shadowSyllables.each(function (i) {
      var correspondingRealSyllable = $(
        "#prosody-real-" + shadowSyllables[i].id.substring(15)
      );
      var correspondingRealSyllableWidth =
        correspondingRealSyllable.innerWidth();
